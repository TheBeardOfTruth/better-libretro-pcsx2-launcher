 ...
 * Once the core has run, attempt to close every frame
 * which will succeed when PCSX2 has stopped.
 */
void retro_run(void)
{
   // Clear the display.
   ...
   // Shutdown the environment since PCSX2 has exited.
...
{
   //this is a monkeybrained solution but 3 am coding go BRRR
   ...
   char cmdl[256] = "-fullscreen -bigpicture";
   ...
   for(int i = 0; i < 4; i++) {
      ...
      {
         printf("libretro-pcsx2-launcher: %s not found.\n", basename[i]);
      }
   }
   printf("libretro-pcsx2-launcher: Failed to start PCSX2. Not installed?\n");
   return false;
}
